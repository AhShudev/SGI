...
class MyLoader {
    ...
    /**
     * Loads a specific node and its children from the graph.
     * @param {Object} graph - YASF file graph.
     * @param {string} nodeId - Node ID to load.
     * @returns {Object3D} Three.js object created.
     */
    loadNode(graph, nodeId) {
        return this.buildNode(graph, nodeId);
    }
    ...
    buildNode(graph, nodeId, transforms = null) {
        ...
        // Store the object in the node dictionary
    ...
    createNurbs(parameters, material) {
        ...
        if (!degree_u || !degree_v || !parts_u || !parts_v || !controlpoints) {
            console.error("Incomplete NURBS parameters:", parameters);
