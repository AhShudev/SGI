...
import { MyReader } from './gameItens/MyReader.js';
...
class MyContents {
    ...
    */
    constructor(app) {
        ...
        // Spriteesheet texture
        const textureLoader = new THREE.TextureLoader();
        this.spriteSheetTexture = textureLoader.load('scene/textures/spritesheet.png');
    ...
    onAfterSceneLoadedAndBeforeRender(data) {
        ...
        // Load objects from the scene
        this.gameReader = new MyReader(this.app.scene, this.materials);
        this.gameReader.loadFromYASF(data.yasf.graph);

        // Update GUI Interface
        this.app.gui.updateGUI();

        // Call Main Menu
        this.menu = new MyMenu(this.app, this.gameReader, this.spriteSheetTexture);
    }
    ...
    */
    ...
    loadTextures(textureData) {
        ...
        for (const key in textureData) {
            ...
            if (data.isVideo) {
                ...
                video.muted = true; // No audio by default
                video.play();
                ...
                baseTexture.mipmaps = mipmaps; 
                baseTexture.generateMipmaps = false; // Prevents automatic generation
    ...
    loadMaterials(materialData, textures) {
        ...
        for (const key in materialData) {
            ...
            const color = matInfo.color ? new THREE.Color(matInfo.color.r, matInfo.color.g, matInfo.color.b) : new THREE.Color(1, 1, 1); 
            const specular = matInfo.specular ? new THREE.Color(matInfo.specular.r, matInfo.specular.g, matInfo.specular.b) : new THREE.Color(0.5, 0.5, 0.5); 
            const emissive = matInfo.emissive ? new THREE.Color(matInfo.emissive.r, matInfo.emissive.g, matInfo.emissive.b) : new THREE.Color(0, 0, 0); 

            const material = new THREE.MeshPhongMaterial({
                color: color,
                specular: specular,
                emissive: emissive,
                shininess: matInfo.shininess !== undefined ? matInfo.shininess : 30,
                ...
                flatShading: matInfo.shading || false, 
                ...
                side: matInfo.twosided ? THREE.DoubleSide : THREE.FrontSide, 
            });
     ...
     */
    addLights() {
        const createDirectionalLight = (position, targetPosition, helperSize) => {
        ...
        // Create and add the two lights
        this.directionalLight1 = createDirectionalLight([150, 300, -150], [0, 0, 0], 50);
        this.directionalLight2 = createDirectionalLight([-150, 300, 150], [0, 0, 0], 50);
    }
